...
export default function P_Mahallistesi() {
  ...
  let toplam = 0
  _3_poz_width.split(" ").map(item => {
    let gecici = Number(item.replace("rem", ""))
    toplam = toplam + gecici
  })
  const total_poz_width = toplam + "rem"
      ...
      {show == "MahalMain" && isProject?.lbs.filter(item => item.openForMahal).length > 0 &&
        <Stack sx={{ width: '100%', padding: "1rem" }} spacing={0}>
          ...
          <Grid sx={{
            ...
            fontWeight: "600", lineHeight: "2rem", height: "2rem", alignItems: "center"
          }} >

            {/* _3_mahal_width -- 1 */}
            <Grid item sx={{ backgroundColor: "lightgray", width: "100%", height: "100%" }}>
            ...
            {/* _3_mahal_width -- 2 */}
            <Grid item sx={{ backgroundColor: "lightgray", textAlign: "center" }}>
              Mahal Tanımı
            </Grid>
            ...
            isProject.lbs.filter(item => item.openForMahal === true).map(lbsOne => (
              ...
              <Box key={lbsOne._id.toString()}>
                ...
                <Grid
                  ...
                  container spacing={0}
                  sx={{
                    display: "grid", gridAutoFlow: "column", gridTemplateColumns: total_mahal_width + " " + _1_bosluk_width + " " + total_poz_width,
...

                {
                  mahaller?.filter(item => item._lbsId.toString() == lbsOne._id.toString()).map((item, index) => {
                    ...
                    // mahal alanı - opsiyon-1 --> 1 den fazla mahal varsa son mahal hariç
                    if (cOunt !== 1 && index + 1 !== cOunt) {
                      return (

                        <Grid key={index} onClick={() => handleSelectMahal(item)} sx={{
                          cursor: "pointer",
                          display: "grid", gridTemplateColumns: _3_mahal_width + " " + _1_bosluk_width + " " + _3_poz_width,
                          "&:hover .hoverTheLbs": {
                            // display: "inline"
                            visibility: "visible"
                          },
                    ...
                    // mahal alanı - opsiyon-2 --> 1 den fazla mahal varsa son mahal
                    if (cOunt !== 1 && index + 1 == cOunt) {
                      return (
                    ...
                    // mahal alanı - opsiyon-3 --> 1 mahal varsa
                    if (cOunt == 1) {
                      return (

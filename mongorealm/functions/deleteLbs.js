exports = async function({projectId,lbsId}){
  ...
  if(lbs.openForMahal) {
    throw new Error("MONGO // deleteLbs // Mahal eklemeye açık başlıklar silinemez, öncelikle mahal eklemeye kapatınız.")
  }
  ...
  if(project.lbs.find(item => item.code.indexOf(text) === 0)) {
    throw new Error("MONGO // deleteLbs // " + "Silmek istediğiniz  Lbs'in alt seviyeleri mevcut, öncelikle onları silmelisiniz.") 
  }

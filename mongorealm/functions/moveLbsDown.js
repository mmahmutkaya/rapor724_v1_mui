exports = async function({projectId,lbsId}){
  ...
  if(!mailTeyit) throw new Error("MONGO // moveLbsDown // Öncelikle üyeliğinize ait mail adresinin size ait olduğunu doğrulamalısınız, tekrar giriş yapmayı deneyiniz veya bizimle iletişime geçiniz.")
  ...
  if(!project) throw new Error("MONGO // moveLbsDown // Proje bulunamadı")
  if(!project.lbs) throw new Error("MONGO // moveLbsDown // Projeye ait Lbs bulunamadı")


  let lbsOne = project.lbs.find(item => item._id.toString() == lbsId.toString())
  
  if(!lbsOne) {
    throw new Error("MONGO // moveLbsDown // lbsId sistemde bulunamadı, sayfayı yenileyiniz, sorun devam ederse Rapor7/24 ile irtibata geçiniz.")
  }

exports = async function({projectId,wbsId}){
  ...
  if(wbs.openForPoz) {
    throw new Error("MONGO // deleteWbs // Poz eklemeye açık başlıklar silinemez, öncelikle poz eklemeye kapatınız.")
  }
  ...
  if(project.wbs.find(item => item.code.indexOf(text) === 0)) {
    throw new Error("MONGO // deleteWbs // " + "Silmek istediğiniz  WBS'in alt seviyeleri mevcut, öncelikle onları silmelisiniz.") 
  }
